...
const TYPESCRIPT_LIB_SRC  = "tsc-bundle --project ./src/tsconfig.json"
const TYPESCRIPT_DEMO_SRC = "tsc-bundle --project ./demo/tsconfig.json"
...
const clean = async () => {
  ...
  await shell("shx rm -rf ./demo/index.js")
  await shell("shx rm -rf ./node_modules")
}

const build = async () => {
  await shell("npm install")
  await shell(`${TYPESCRIPT_LIB_SRC}`)
}

const start = async () => {
  await shell("npm install")
  await shell(`${TYPESCRIPT_DEMO_SRC}`)
  await Promise.all([
    shell(`${TYPESCRIPT_DEMO_SRC} --watch > /dev/null`),
    shell("fsweb ./demo 5000")
  ])
}
